...
AION <- na.omit(read_excel("C:/Econometria/AION.xlsx"))

AION2 <-  ts(AION$Fechar, start = 2017, frequency = 365)
...
acf(AION$Fechar,lend=2, lwd=5,col="darkblue",main= "Função Autocorrelação - FAC")              #Melhorando aspecto da FAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")

pacf(AION$Fechar,lend=60, lwd=5,col="darkblue",main= "Função Autocorrelação Parcial - FACP")   #Melhorando aspecto da PAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")

#Teste ADF
ur.df(AION2, "none", lags = 1)
